...
void evaluateTick() {
    ...
    for (int y = g.h - 2; y >= 0; --y) {
        ...
        for (int xi = 0; xi < g.w; ++xi) {
            ...
            // ------------------------------------------------------------------
            // Life check — auto-decrement built-in life counter; destroy if zero.
            // Runs before any rules so death is immediate and unconditional.
            // ------------------------------------------------------------------
            if (def && def->lifespan > 0) {
                uint16_t& life = g_grid.vars_write[i * NUM_VARS_PER_CELL + LIFE_VAR_SLOT];
                if (life > 0) --life;
                if (life == 0) {
                    g_grid.write[i] = EMPTY_ID;
                    g_grid.dirty[i] = true;
